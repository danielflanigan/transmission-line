...
def main(output_directory):
    """Draw a design using `transmission_line/cpw.py` and save it in the given directory as `cpw.gds`."""
    ...
    single_cpw.draw(cell=main_cell,  # Draw the CPW into this cell
                    origin=(100, 0),  # The points are drawn relative to this point, so this CPW starts at (200, 0)
                    layer=0,
    ...
    single_cpw.draw(cell=main_cell,
                    origin=(700, -100),  # This CPW starts at (700, -100) + (100, 0) = (800, -100)
                    layer=1)
    ...
    # To place the start of the center trace away from the edge of the chip, a good practice to avoid shorts, start with
    # a section of CPW that will be drawn with `draw_trace` set to False.
    ...
    # last; for these, draw only the grounds to form a gap between the end of the launch and the edge of the chip.
    feedline.draw(cell=main_cell, origin=(-3000, 0), layer=0,
                  draw_trace=True,
                  draw_gap=False,
                  draw_ground=True,
                  individual_keywords={0: dict(draw_trace=False),  # left_border
                                       6: dict(draw_trace=False)},  # upper_border
                  )

    # Draw two negative CPWs with an elbow coupler next to the feedline. This structure is a section of CPW that has
    # exactly three points and by default draws a rounded cap that avoids sharp edges.
    ...
    # update the design quickly. Because the elbow points given beloware (0, 0), that point will be placed at the given
    ...
    # This CPW has a short at the far end, because we are drawing the negative space and the CPW simply ends, so it is a
    # quarterwave resonator.
    quarterwave_cpw = tl.SegmentList([
        cpw.CPWElbowCoupler(open_point=(0, 200),  # The tip of the coupler
                            ...
                            # Use default open_at_start=True because this is the first Segment
                            ),
        cpw.CPW(outline=[(0, 0), (500, 0), (500, 500)], trace=coupled_cpw_trace, gap=coupled_cpw_gap)
    ])

    quarterwave_cpw.draw(cell=main_cell, origin=(coupled_cpw_distance, 300), layer=2)

    # This CPW has an open at the far end that is drawn by the CPWRoundedOpen, so it is a halfwave resonator.
    halfwave_cpw = tl.SegmentList([
        cpw.CPWElbowCoupler(open_point=(200, 0),
                            elbow_point=(0, 0),
                            joint_point=(0, 100),
                            trace=coupled_cpw_trace,
                            gap=coupled_cpw_gap),
